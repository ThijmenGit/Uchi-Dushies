...
portfolio = []
df = pd.Dataframe(portfolio, columns = ["Name", "Region", "Currency", "Euros "])
...
def buy_stock():
    try:
        ...
        stock_to_buy = {"Name": "Apple", "Region": "Amsterdam", "Currency": "euros","last_price": 350}  ## getstockprice()
        for i in range(amount):
            portfolio.append(f"{stock_to_buy}")
        transaction_price = stock_to_buy["last_price"] * amount
...
if outcome == True:
    print("Got it")
    buy_stock()
    print("Current portfolio holds", portfolio)
elif outcome == False:
